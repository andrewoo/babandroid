...

{
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			start_x = stop_x - this.getLeft();// 用(int)event.getX();//
												// 一样,表示相对于当前点击Widget（控件）左上角的坐标，这里就是相对于自定义imageView左上角的坐标.建议用前者，如果不是全屏拖动，而是指定范围内，一样适用！
		...
		case MotionEvent.ACTION_UP:
			...
			if (getHeight() <= screenH) {
				...
				if (this.getTop() < 0) {
					...
					this.layout(this.getLeft(),

					screenH - getHeight(), this.getRight(), screenH);
				}
...

				{
					...
					this.layout(this.getLeft(), 0,

					this.getRight(), 0 + this.getHeight());
	...
	private void setScale(float temp, int flag) {
		...
		if (flag == BIGGER) {
			...
			this.getTop() + (int) (temp * this.getHeight()), this.getRight()
